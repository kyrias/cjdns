...
static void authorizedPasswords(List* list, struct Context* ctx)
{
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        Dict* d = List_getDict(list, i);
        String* passwd = Dict_getString(d, String_CONST("password"));
        String* login = Dict_getString(d, String_CONST("login"));
        ...
        Log_info(ctx->logger, "Adding authorized password #[%d] for login [%s].",
            i, displayName->bytes);
        ...
        if (login) {
            Dict_putString(args, String_CONST("login"), login, child);
        }
        if (peerName) {
